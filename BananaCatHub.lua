...
-- ==========================================
Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/phucthang-glitch/toiyeuem/refs/heads/main/ui_BananaHub_pink_blue.lua.txt"))()
...
-- ==========================================
Tabs = {
    ["Info"]     = wrapTab(Window:AddTab("ℹ️ Thông Tin")),
    ["Hop"]      = wrapTab(Window:AddTab("🌐 Hop Server")),
    ["Main"]     = wrapTab(Window:AddTab("⚔️ Cày Cấp")),
    ["Sea"]      = wrapTab(Window:AddTab("🎉 Sự Kiện")),
    ["Item"]     = wrapTab(Window:AddTab("🎁 Lấy & Nâng Cấp")),
    ["Setting"]  = wrapTab(Window:AddTab("⚙️ Cài Đặt")),
    ["Status"]   = wrapTab(Window:AddTab("📡 Webhook")),
    ["Stats"]    = wrapTab(Window:AddTab("📊 Chỉ Số")),
    ["Player"]   = wrapTab(Window:AddTab("👤 Người Chơi")),
    ["Teleport"] = wrapTab(Window:AddTab("✈️ Dịch Chuyển")),
...
-- ==========================================
-- SEA FLAGS - detect biển hiện tại
-- Ưu tiên: PlaceId → fallback Map trong Workspace
-- Không bao giờ kick, luôn tự detect được biển
-- ==========================================
Sea1 = false
Sea2 = false
Sea3 = false
...
-- Danh sách PlaceId đã biết (bao gồm cả private server place)
local SEA1_IDS = {2753915549, 6284583030}
local SEA2_IDS = {4442272183}
local SEA3_IDS = {7449423635}
...
if tableHas(SEA1_IDS, v5) then
    Sea1 = true
    print("🌊 Sea 1 - First Sea (PlaceId: " .. v5 .. ")")
elseif tableHas(SEA2_IDS, v5) then
    Sea2 = true
    print("🌊 Sea 2 - Second Sea (PlaceId: " .. v5 .. ")")
elseif tableHas(SEA3_IDS, v5) then
    Sea3 = true
    print("🌊 Sea 3 - Third Sea (PlaceId: " .. v5 .. ")")
else
    -- Fallback: detect qua nội dung Workspace/Map
    print("⚠️ PlaceId " .. tostring(v5) .. " không có trong danh sách, đang tự detect biển...")
    local map = workspace:FindFirstChild("Map")
    if map then
        -- Sea 3: có Floating Turtle hoặc Cake Island
        if map:FindFirstChild("FloatingTurtle") or map:FindFirstChild("Floating Turtle")
        or map:FindFirstChild("CakeIsland") or map:FindFirstChild("Cake Island")
        or map:FindFirstChild("HauntedCastle") or map:FindFirstChild("Haunted Castle") then
            Sea3 = true
            print("🌊 Detected: Sea 3 (qua Workspace.Map)")
        -- Sea 2: có Dressrosa hoặc Punk Hazard
        elseif map:FindFirstChild("Dressrosa") or map:FindFirstChild("PunkHazard")
        or map:FindFirstChild("Punk Hazard") or map:FindFirstChild("ZombieIsland")
        or map:FindFirstChild("Zombie Island") then
            Sea2 = true
            print("🌊 Detected: Sea 2 (qua Workspace.Map)")
        else
            Sea1 = true
            print("🌊 Detected: Sea 1 (fallback)")
        end
    else
        -- Không có Map, detect qua level player
        ...
        if ok and lvl then
            if lvl >= 1500 then
                Sea3 = true
                print("🌊 Detected: Sea 3 (qua Level " .. lvl .. ")")
            elseif lvl >= 700 then
                Sea2 = true
                print("🌊 Detected: Sea 2 (qua Level " .. lvl .. ")")
            else
                Sea1 = true
                print("🌊 Detected: Sea 1 (qua Level " .. lvl .. ")")
            end
        else
            Sea1 = true
            print("🌊 Fallback cuối: Sea 1")
        end
    end
end
    ...
    while task.wait() do
        if _G.AutoLevel then
            pcall(function()
                ...
                -- Chưa có quest → teleport thẳng đến NPC nhận quest
                if not questVisible then
                    bringmob = false
                    -- Teleport trực tiếp (không dùng Tween để tránh bay vòng vòng)
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrameQ
                    task.wait(0.3)
                    if (CFrameQ.Position - game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 50 then
                ...
                if questVisible and string.find(QuestTitle, NameMon) then
                    if game:GetService("Workspace").Enemies:FindFirstChild(Ms) then
                        for _, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                            if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 and v.Name == Ms then
                                ...
                                repeat
                                    ...
                                    -- Teleport thẳng cạnh quái (không Tween)
                        ...
                        bringmob = false
                        -- Teleport thẳng đến vị trí spawn quái (không Tween để tránh bay vòng vòng)
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrameMon
                        task.wait(0.1)
                        -- Nếu quái trong ReplicatedStorage (boss/special), teleport tới đó
                        if game:GetService("ReplicatedStorage"):FindFirstChild(Ms) then
                            pcall(function()
                                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game:GetService("ReplicatedStorage"):FindFirstChild(Ms).HumanoidRootPart.CFrame * CFrame.new(0, 20, 0)
                            end)
                        end
                    end
                end
...
-- ==========================================
-- ĐẢO BÍ ẨN & TRĂNG TRÒN - hiển thị cho tất cả các biển
-- ==========================================
FullMoonStatus = "Đang tải..."
MirageStatus = "Đang tải..."

Tabs.Sea:AddSection("Đảo Bí Ẩn & Trăng Tròn")
local vu658 = Tabs.Sea:AddParagraph({
    ["Title"] = "🌕 Trạng Thái",
    ["Description"] = "Đang tải...",
})

-- Theo dõi trạng thái Trăng Tròn qua MoonTextureId
-- 5 texture tương ứng 5 pha: 0% → 25% → 50% → 75% → 100%
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local v659 = game:GetService("Lighting").Sky.MoonTextureId
            -- Thứ tự pha trăng theo Blox Fruits (từ nhỏ đến lớn)
            if v659 == "http://www.roblox.com/asset/?id=9709149431" then
                FullMoonStatus = "🌕 100% (Trăng Tròn!)"
...
-- Theo dõi Đảo Bí Ẩn qua MysticIsland trong Map
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            local map = game.Workspace:FindFirstChild("Map")
            if map and map:FindFirstChild("MysticIsland") then
                MirageStatus = "✅ Đã xuất hiện!"
            else
                MirageStatus = "❌ Chưa xuất hiện"
            end
        end)
    end
end)
...
task.spawn(function()
    while task.wait(1) do
        pcall(function()
            vu658:SetDesc(
                "🏝️ Đảo Bí Ẩn: " .. MirageStatus ..
                "\n🌙 Trăng Tròn: " .. FullMoonStatus
            )
        end)
    end
end)

-- Nút Bay Đến Chợ Cao (chỉ hoạt động khi đảo xuất hiện)
Tabs.Sea:AddButton({
    ["Title"] = "🏝️ Bay Đến Chợ Cao (Mirage)",
    ["Callback"] = function()
        TweenToHighestPoint()
    end
})
...
function TweenToHighestPoint()
    ...
    if v660 then
        ...
        warn("[HDanh Hub] Đảo Bí Ẩn chưa xuất hiện!")
    end
end

function getHighestPoint()
    local map = game.Workspace:FindFirstChild("Map")
    if not map or not map:FindFirstChild("MysticIsland") then
        return nil
    end
...
if Sea3 then
    -- Các tính năng riêng Sea3 (nếu có thêm sau)
end
...
do
    ...
    local function _ListServers(cursor)
        local url = "https://games.roblox.com/v1/games/" .. _PlaceId .. "/servers/Public?sortOrder=Asc&limit=100"
    ...
    local function _FindBestServer()
        local cursor = nil
        local best = nil
        repeat
            local data = _ListServers(cursor)
            for _, server in ipairs(data.data) do
                if server.playing > 0 and server.playing < server.maxPlayers then
                    if server.playing == 1 then return server end
    ...
    local function _AutoRetryHop()
        task.spawn(function()
            while true do
                local target = _FindBestServer()
                if target then
                    local ok, err = pcall(function()
                        _TeleportService:TeleportToPlaceInstance(_PlaceId, target.id, _LocalPlayer)
                    end)
                    if ok then break end
                    warn("Teleport that bai: " .. tostring(err) .. " -> thu lai sau 5s")
                else
                    warn("Khong tim duoc server phu hop -> thu lai sau 5s")
                end
                task.wait(5)
            end
        end)
    ...
    Tabs.Hop:AddSection("Hop Server It Nguoi")
    Tabs.Hop:AddButton({
        ["Title"] = "🔄 Hop Uu Tien Server 1 Nguoi",
        ["Callback"] = function()
            _AutoRetryHop()
        end
    })
    Tabs.Hop:AddSection("Thong Tin")
    Tabs.Hop:AddParagraph({
        Title = "Cach dung",
        Description = "Nhan nut tren de tu dong tim va teleport sang server it nguoi nhat (uu tien server chi co 1 nguoi). Neu that bai se tu retry sau 5 giay."
    })
end

-- ===============================================================
-- ========== HOP SERVER SỰ KIỆN (Full Moon, Haki, Sword, Mirage, Prehistoric) ==========
-- Logic dùng API: http://n1.hanamc.io.vn:25568/qvuong/api/{name}
-- Trả về danh sách Job_id + place_id để teleport vào đúng server sự kiện
-- ===============================================================
do
    local _reqfunc = request or (http and http.request) or http_request
    local _httpser = game:GetService("HttpService")
    local _tpser   = game:GetService("TeleportService")
    local _lp      = game:GetService("Players").LocalPlayer

    -- Lấy danh sách server sự kiện từ API
    local function _GetEventJobs(eventName)
        if not _reqfunc then
            warn("[HDanh Hub] Executor không hỗ trợ HTTP request")
            return nil
        end
        local ok, res = pcall(function()
            return _reqfunc({
                Url    = "http://n1.hanamc.io.vn:25568/kaibeo/api/" .. tostring(eventName),
                Method = "GET"
            })
        end)
        if not (ok and res) then
            warn("[HDanh Hub] Không thể gọi API: " .. tostring(eventName))
            return nil
        end
        local dok, decoded = pcall(function()
            return _httpser:JSONDecode(res.Body)
        end)
        if not dok or not decoded then
            warn("[HDanh Hub] Lỗi parse JSON cho: " .. tostring(eventName))
            return nil
        end
        local jobs = {}
    ...
    -- Hop vào server sự kiện theo tên API
    local function _HopEvent(eventName)
        task.spawn(function()
            local jobs = _GetEventJobs(eventName)
            if not jobs or #jobs < 1 then
                warn("[HDanh Hub] Không tìm được server: " .. tostring(eventName))
                return
            end
            local hopped = false
            for _, v in ipairs(jobs) do
                if v and v.Job_id and v.place_id then
                    if tonumber(game.PlaceId) == tonumber(v.place_id) then
                        local ok, err = pcall(function()
                            _tpser:TeleportToPlaceInstance(
                                tonumber(v.place_id),
                                tostring(v.Job_id),
                                _lp
                            )
                        ...
                        if ok then
                            ...
                            warn("[HDanh Hub] Teleport thất bại: " .. tostring(err))
                        end
                        task.wait(1)
                    end
                end
            ...
            if not hopped then
                warn("[HDanh Hub] Không tìm được server phù hợp cho: " .. tostring(eventName))
            end
        end)
    end

    Tabs.Hop:AddSection("Hop Server Sự Kiện")

    Tabs.Hop:AddButton({
        ["Title"] = "🏝️ Hop Mirage Island",
        ["Callback"] = function()
            _HopEvent("mirage")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Shizu",
        ["Callback"] = function()
            _HopEvent("swordshizu")
        end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Saishi",
        ["Callback"] = function()
            _HopEvent("swordsaishi")
        end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚔️ Hop Sword Oroshi",
        ["Callback"] = function()
            _HopEvent("swordoroshi")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ...
        ["Callback"] = function()
            _HopEvent("hakisnowwhite")
        end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "⚡ Hop Haki Legendary",
        ["Callback"] = function()
            _HopEvent("haki")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ["Title"] = "🏴‍☠️ Hop Pirate Raid",
        ["Callback"] = function()
            _HopEvent("pirateraid")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ...
        ["Callback"] = function()
            _HopEvent("tyrantskies")
        end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🌫️ Hop Grey Beard",
        ["Callback"] = function()
            _HopEvent("greybeard")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ...
        ["Callback"] = function()
            _HopEvent("soulreaper")
        end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🎂 Hop Cake Queen",
        ["Callback"] = function()
            _HopEvent("cakequeen")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ["Title"] = "🎂 Hop Cake Prince",
        ["Callback"] = function()
            _HopEvent("cakeprince")
        end
    })
    ...
    Tabs.Hop:AddButton({
        ["Title"] = "🦕 Hop Prehistoric",
        ["Callback"] = function()
            _HopEvent("prehistoric")
        end
    })

    Tabs.Hop:AddButton({
        ["Title"] = "🧊 Hop Frozen Leviathan",
        ["Callback"] = function()
            _HopEvent("frozenleviathan")
        end
    })

    Tabs.Hop:AddSection("Lưu Ý")
    Tabs.Hop:AddParagraph({
        Title = "Cách dùng Hop Sự Kiện",
        Description = "Nhấn nút tương ứng để tự động tìm và vào server đang có sự kiện đó. Yêu cầu executor hỗ trợ HTTP request (Delta, Velocity,...). Nếu không có server sẽ báo lỗi trong console."
    })
end

-- ===============================================================
print('HDanh Hub - Tat ca tinh nang da duoc load!')
